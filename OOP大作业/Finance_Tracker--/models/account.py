...
class Transaction(ABC):
    ...
    def __init__(self, amount: float, date: datetime, description: str, category: Optional[str] = None):
        ...
        self._transaction_id = str(uuid.uuid4())[:8]  # 生成唯一ID
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Transaction':
        ...
        if data['type'] == 'income':
            return Income(data['amount'], date, data['description'], data.get('category'))
        elif data['type'] == 'expense':
            return Expense(data['amount'], date, data['description'], data.get('category'))
        else:
